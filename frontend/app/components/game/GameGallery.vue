...
<template>
  <section class="grid grid-cols-2 py-8 gap-5">
    ...
    />
    <div
      v-for="(screenshot, index) in screenshots.slice(0, maxScreenshots)"
      :key="screenshot.id"
      class="relative group"
      :class="{ 'col-span-2': !trailer && index === 0 }"
    >
      <img
        :src="screenshot.path_thumbnail"
        :alt="`Screenshot ${screenshot.id}`"
        class="w-full object-cover rounded-lg transition cursor-pointer hover:scale-110"
        @click="openScreenshot(index)"
        :class="{
          'brightness-30 group-hover:brightness-40':
            index === maxScreenshots - 1,
          'aspect-video': !trailer && index === 0,
          'h-40': trailer || index !== 0,
        }"
      />
      <button
        v-if="index === maxScreenshots - 1"
        type="button"
...
const maxScreenshots = computed(() => (props.trailer ? 4 : 5));

const openScreenshot = (index) => {
  if (index < maxScreenshots.value - 1) {
